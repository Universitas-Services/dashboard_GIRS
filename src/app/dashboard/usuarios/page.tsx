...
export default function UsuariosPage() {
  ...
  const ejecutarEliminacionMasiva = async () => {
    ...
    try {
      ...
      toast.success(
        `${selectedUserIds.length} usuarios eliminados exitosamente.`
      );
  ...
  const fetchUsers = useCallback(
    async (currentFilters: GetUsersParams, signal?: AbortSignal) => {
      setLoading(true);
      try {
  ...
  useEffect(() => {
    if (isInitialLoad) {
      ...
      if (filters.tipoUsuario) {
        params.set('tipoUsuario', filters.tipoUsuario);
      } else {
        params.delete('tipoUsuario');
      }
      if (filters.estadoCuenta) {
        params.set('estadoCuenta', filters.estadoCuenta);
      } else {
        params.delete('estadoCuenta');
      }
      if (filters.estado) {
  ...
  const handleMunicipioChange = (municipioNombre: string) => {
    setFilters((prev) => ({
      ...prev,
      municipio: municipioNombre === 'todos' ? undefined : municipioNombre,
      page: 1,
    }));
  };
  ...
  const statCards = [
    ...
    },
    {
      ...
      value:
        dashboardData?.analytics.porTipousuario.servidoresPublicos.toLocaleString() ||
        '...',
      icon: Building2,
      color: '#10b981',
      bgColor: '#dcfce7',
    ...
    },
    {
      ...
      value:
        dashboardData?.analytics.porTipousuario.asesoresPrivados.toLocaleString() ||
        '...',
      icon: Shield,
      color: '#3b82f6',
      bgColor: '#dbeafe',
    ...
    },
    {
      ...
      value:
        dashboardData?.analytics.usuariosNoVerificados.toLocaleString() ||
        '...',
      icon: ClipboardList,
      color: '#b45309',
      bgColor: '#fef3c7',
    ...
    },
    {
      title: 'SUSPENSIONES',
      value:
        dashboardData?.analytics.suspensionesRecientes.toLocaleString() ||
        '...',
      icon: Ban,
      color: '#be123c',
      bgColor: '#ffe4e6',
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex flex-col">
          ...
          <p className="text-muted-foreground text-xs font-medium mt-0.5">
            Gestión integral del ecosistema de servidores y asesores.
          </p>
        </div>
      ...
      <div className="mt-4 flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            {/* Tarjeta de selects */}
            <div
              className="lg:col-span-2 rounded-xl p-6 flex flex-col sm:flex-row gap-6 justify-between items-center"
              style={{ backgroundColor: 'var(--admin-filter-bg)' }}
            >
              <div className="flex flex-col w-full">
                <label className="text-[11px] font-bold tracking-wider text-muted-foreground uppercase mb-2">
                  Estado
                </label>
                <Select
              ...
              <div className="flex flex-col w-full">
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="seleccionar" disabled>
                      Seleccionar Municipio
                    </SelectItem>
            ...
            {/* Tarjeta de perfil */}
            <div
              className="lg:col-span-1 rounded-xl p-6 flex flex-col justify-center items-center"
              style={{ backgroundColor: 'var(--admin-filter-bg)' }}
            >
              <div className="w-fit">
                ...
                <div className="flex gap-2">
                  ...
                  <Badge
                    ...
                    style={
                      filters.tipoUsuario === 'SERVIDOR_PUBLICO'
                        ? {
                            backgroundColor: 'var(--admin-toggle-active-bg)',
                            color: 'var(--admin-toggle-active-text)',
                          }
                  ...
                  <Badge
                    ...
                    style={
                      filters.tipoUsuario === 'ASESOR_PRIVADO'
                        ? {
                            backgroundColor: 'var(--admin-toggle-active-bg)',
                            color: 'var(--admin-toggle-active-text)',
                          }
          ...
          <div
            className="rounded-xl p-6 flex flex-col md:flex-row justify-between items-center gap-6"
            style={{ backgroundColor: 'var(--admin-filter-bg)' }}
          >
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-b border-gray-200/60 hover:bg-transparent">
                  ...
                  <TableHead className="text-[10px] font-bold tracking-widest text-muted-foreground uppercase h-12 w-[250px]">
                    Usuario / ID
                  </TableHead>
                  <TableHead className="text-[10px] font-bold tracking-widest text-muted-foreground uppercase h-12 w-[200px]">
                    Rol de Perfil
                  </TableHead>
                  <TableHead className="text-[10px] font-bold tracking-widest text-muted-foreground uppercase h-12 w-[180px]">
                    Ubicación
                  </TableHead>
                            ...
                            <div className="flex items-start gap-3">
                              ...
                              <div className="flex flex-col">
                                <h4 className="font-bold text-slate-900 text-sm">
                                  Eliminar {selectedUserIds.length} Usuarios
                                </h4>
                                <p className="text-xs text-slate-500 mt-0.5">
                                  Esta acción masiva es irreversible.
                                </p>
                              </div>
                            </div>
                  ...
                  users.map((user) => {
                    return (
                      ...
                      >
                        ...
                        <TableCell className="py-5 max-w-[280px]">
                          ...
                          >
                            <Avatar className="h-11 w-11 border border-slate-200/60 shadow-sm bg-slate-100 shrink-0">
                              ...
                              />
                              <AvatarFallback className="font-bold text-slate-800 text-xs">
                                {user.nombre[0]}
                            ...
                            <div className="flex flex-col min-w-0">
                              <span
                                className="font-bold text-slate-900 text-sm hover:underline truncate"
                                title={`${user.nombre} ${user.apellido || ''}`}
                              >
                                {user.nombre} {user.apellido}
                              </span>
                              <span
                                className="text-[11px] text-muted-foreground mt-0.5 font-medium break-all whitespace-normal line-clamp-2 leading-tight"
                                title={user.email}
                              >
                                {user.email}
                              </span>
                        ...
                        <TableCell className="py-5">
                          {user.isActive ? (
                            <Badge className="px-3 py-1 text-[10px] font-extrabold rounded-md shadow-none cursor-default border-none bg-emerald-100 text-emerald-700 hover:bg-emerald-100">
                              ACTIVO
                            </Badge>
                          ) : (
                            <Badge className="px-3 py-1 text-[10px] font-extrabold rounded-md shadow-none cursor-default border-none bg-rose-100 text-rose-700 hover:bg-rose-100">
                              INACTIVO
                            </Badge>
                          )}
                        </TableCell>
                        <TableCell className="py-5">
                          {(() => {
                            const config = statusConfig[user.estadoCuenta] || {
                              label: user.estadoCuenta || 'Sin Estado',
                              color: '#64748b',
                              bgColor: '#f1f5f9',
                            };
                            return (
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between px-6 py-5 border-t border-gray-200/60 bg-[var(--admin-filter-bg)]">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-6">
              <span className="text-xs font-semibold text-muted-foreground">
                ...
                de{' '}
                <span className="text-slate-900">{meta?.totalItems || 0}</span>{' '}
              ...
              <div className="flex items-center gap-1.5">
                    ...
                    (_, i) => {
                      ...
                      return (
                        <Button
                          key={pageNum}
                          variant={
                            filters.page === pageNum ? 'default' : 'ghost'
                          }
                          size="icon"
                ...
                {meta && meta.totalPages > 5 && (
                  <span className="text-sm font-bold text-slate-400 px-1">
                    ...
                  </span>
                )}
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-6 pb-12">
          {statCards.map((stat, idx) => {
            ...
            const isActive =
              filter &&
              ((filter.type === 'tipo' &&
                filters.tipoUsuario === filter.value) ||
                (filter.type === 'estadoCuenta' &&
                  filters.estadoCuenta === filter.value) ||
                (filter.type === 'reset' &&
                  !filters.tipoUsuario &&
                  !filters.estadoCuenta));
            ...
            return (
              <Card
                ...
                style={
                  isActive && filter
                    ? { borderColor: stat.color, backgroundColor: 'white' }
                    : {}
                }
